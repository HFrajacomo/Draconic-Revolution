...
STATES:
	...
	5 = Terra Y-
*/
public class TerraCrystal_Object : BlocklikeObject
{
	private VisCrystalBehaviour behaviour;

	public TerraCrystal_Object(bool isClient){
		this.shaderIndex = ShaderIndex.ASSETS_SOLID;
		this.name = "Terra Crystal";
		this.solid = true;
		this.transparent = 1;
		...
		this.maxHP = 100;
		this.atlasPosition = new int2(3,0);
		this.customPlace = true;
		this.behaviour = new VisCrystalBehaviour((ushort)BlockID.TERRA_CRYSTAL);

		if(isClient){
			this.go = GameObject.Find("----- PrefabObjects -----/TerraCrystal_Object");
			this.hitboxObject = GameObject.Find("----- PrefabObjects -----/TerraCrystal_Object/Hitbox");
