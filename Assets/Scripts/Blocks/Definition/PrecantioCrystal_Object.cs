...
STATES:
	...
	5 = Precantio Y-
*/
public class PrecantioCrystal_Object : BlocklikeObject
{
	private VisCrystalBehaviour behaviour;

	public PrecantioCrystal_Object(bool isClient){
		this.shaderIndex = ShaderIndex.ASSETS_SOLID;
		this.name = "Precantio Crystal";
		this.solid = true;
		this.transparent = 1;
		...
		this.maxHP = 100;
		this.atlasPosition = new int2(6,0);
		this.customPlace = true;
		this.behaviour = new VisCrystalBehaviour((ushort)BlockID.PRECANTIO_CRYSTAL);

		if(isClient){
			this.go = GameObject.Find("----- PrefabObjects -----/PrecantioCrystal_Object");
			this.hitboxObject = GameObject.Find("----- PrefabObjects -----/PrecantioCrystal_Object/Hitbox");
