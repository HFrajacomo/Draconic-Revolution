...
public class Lava_Block : Blocks
{
	...
	public override int OnPlace(ChunkPos pos, int x, int y, int z, int facing, ChunkLoader_Server cl){
		...
		message.DirectBlockUpdate(BUDCode.PLACE, pos, thisPos.blockX, thisPos.blockY, thisPos.blockZ, facing, this.lavaCode, cl.chunks[thisPos.GetChunkPos()].metadata.GetState(thisPos.blockX, thisPos.blockY, thisPos.blockZ), cl.chunks[thisPos.GetChunkPos()].metadata.GetHP(thisPos.blockX, thisPos.blockY, thisPos.blockZ));
		...
		if(type == BUDCode.BREAK || type == BUDCode.CHANGE){
			...
			*/
			if(state == 0){
				...
				if(below == 0 || (below == this.lavaCode && ShouldStateOverpower(state, belowState)) || IsWashable(below, cl)){
					...
					if(GetSameLevelAroundCount(myX, myY, myZ, 3, cl) > 0){
						...
						cl.chunks[newPos.GetChunkPos()].data.SetCell(newPos.blockX, newPos.blockY, newPos.blockZ, this.lavaCode);
				...
				// Normal Behaviour
					...
					for(int i=0; i < 8; i+=2){
						...
						// If is water
						else if(this.aroundCodes[i] == lavaCode && ShouldStateOverpower(targetState, this.aroundStates[i])){
						...
						if(found){
							GetDirectionPos(myX, myY, myZ, i);
							cl.chunks[cachedPos.GetChunkPos()].data.SetCell(cachedPos.blockX, cachedPos.blockY, cachedPos.blockZ, this.lavaCode);
			...
			*/
				...
				if(below == this.lavaCode && belowState == 1){
					CastCoord newPos = new CastCoord(new Vector3(myX, myY-1, myZ));
			...
			*/
				...
				if(below == this.lavaCode && belowState == 2){
					CastCoord newPos = new CastCoord(new Vector3(myX, myY-1, myZ));
			...
			*/
				...
				if(this.aroundCodes[cameFromDir[state]] != lavaCode || !cameFromState[state].Contains(this.aroundStates[cameFromDir[state]])){
					this.OnBreak(thisPos.GetChunkPos(), thisPos.blockX, thisPos.blockY, thisPos.blockZ, cl);
					return;
				}
				...
				if(GetSameLevelAroundCount(myX, myY, myZ, 3, cl) >= 2){
					cl.chunks[thisPos.GetChunkPos()].data.SetCell(thisPos.blockX, thisPos.blockY, thisPos.blockZ, this.lavaCode);
				...
				if(below == 0 || below == this.lavaCode && ShouldStateOverpower(20, belowState) || IsWashable(below, cl)){
					CastCoord newPos = new CastCoord(new Vector3(myX, myY-1, myZ));
				...
				else if(below == this.lavaCode && belowState == 20){return;}
			...
			*/
				...
				// If should create falling blocks
				if(below == 0 || below == this.lavaCode && ShouldStateOverpower(21, belowState) || IsWashable(below, cl)){
					CastCoord newPos = new CastCoord(new Vector3(myX, myY-1, myZ));
				...
				// If is falling block pivot
				else if(below == this.lavaCode && belowState == 21){return;}
			...
			*/
				...
				// If should die
				if(above != this.lavaCode || (above == this.lavaCode && (aboveState != 0 && aboveState != 19))){
					this.OnBreak(thisPos.GetChunkPos(), thisPos.blockX, thisPos.blockY, thisPos.blockZ, cl);
					return;					
				}

				// If should create new falling 3s
				if(below == 0 || below == this.lavaCode && ShouldStateOverpower(19, belowState) || IsWashable(below, cl)){
					CastCoord newPos = new CastCoord(new Vector3(myX, myY-1, myZ));
				...
				if(below == this.lavaCode && above == this.lavaCode) {return;}
			...
			*/
				...
				// If should die
				if(above != this.lavaCode || (above == this.lavaCode && (aboveState != 1 && aboveState != 20 && !(aboveState >= 3 && aboveState <= 10)))){
					this.OnBreak(thisPos.GetChunkPos(), thisPos.blockX, thisPos.blockY, thisPos.blockZ, cl);
					return;					
				}
			...
			*/
				...
				// If should die
				if(above != this.lavaCode || (above == this.lavaCode && (aboveState != 2 && aboveState != 21 && !(aboveState >= 11 && aboveState <= 18)))){
					this.OnBreak(thisPos.GetChunkPos(), thisPos.blockX, thisPos.blockY, thisPos.blockZ, cl);
					return;					
				}
	...
	private void GetStateAround(int myX, int myY, int myZ, ChunkLoader_Server cl){
		...
		if(cl.chunks[cachedCoord.GetChunkPos()].data.GetCell(cachedCoord.blockX, cachedCoord.blockY, cachedCoord.blockZ) == this.lavaCode)
	...
	private bool CheckHigherLevelWaterAround(int myX, int myY, int myZ, int currentWaterLevel, ChunkLoader_Server cl){
		for(int i=0; i<8; i++){
			if(this.aroundCodes[i] == this.lavaCode && TranslateWaterLevel(this.aroundStates[i]) > currentWaterLevel){
				return true;
			}				
		}
		return false;
	...
	private int GetHighLevelAroundCount(int x, int y, int z, int currentWaterLevel, ChunkLoader_Server cl, bool nofalling=false){
		...
		if(!nofalling){
			...
			for(int i=0; i<8; i+=2){
				if(this.aroundCodes[i] == this.lavaCode && TranslateWaterLevelSpec(this.aroundStates[i]) > currentWaterLevel){
					count++;
				}
			}			
		}
	...
	private int GetSameLevelAroundCount(int x, int y, int z, int currentWaterLevel, ChunkLoader_Server cl){
		...
		if(currentWaterLevel == 1){
			for(int i=0; i<8; i+=2){
				if(this.aroundCodes[i] == this.lavaCode && this.aroundStates[i] == 2){
					count++;
				}
			}
		}
		if(currentWaterLevel == 2){
			for(int i=0; i<8; i+=2){
				if(this.aroundCodes[i] == this.lavaCode && this.aroundStates[i] == 1){
					count++;
				}
			}
		}
		if(currentWaterLevel == 3){
			for(int i=0; i<8; i+=2){
				if(this.aroundCodes[i] == this.lavaCode && this.aroundStates[i] == 0){
					count++;
				}
			}
		}
	...
	private void EmitWaterBUD(int myX, int myY, int myZ, ChunkLoader_Server cl){
		...
		// Adjascents
		for(int i=0; i<8; i+=2){
			if(this.aroundCodes[i] == this.lavaCode){
	...
	public void Update(CastCoord c, BUDCode type, int facing, ChunkLoader_Server cl){
		...
		this.reloadMessage.DirectBlockUpdate(type, c.GetChunkPos(), c.blockX, c.blockY, c.blockZ, facing, this.lavaCode, cl.GetState(c), ushort.MaxValue);
