...
public static class LiquidMeshData
{
	...
	*/
    ...
    /*
    MULTITHREADED BASED STRUCT
    */

    public static float3[][] floatverts = {

        // State 0 (Still 3)
        new float3[] {new float3(1, 1, 1),
        new float3(-1, 1, 1),
        new float3(-1, -1, 1),
        new float3(1, -1, 1),
        new float3(-1, 1, -1),
        ...
        // State 2 (Still 1)
        new float3[] {new float3(1, -0.5f, 1),
        new float3(-1, -0.5f, 1),
        new float3(-1, -1, 1),
        new float3(1, -1, 1),
        new float3(-1, -0.5f, -1),
    ...
    public static NativeArray<float3> VertsByState(int dir, ushort s, float3 pos, float scale=0.5f){
        NativeArray<float3> fv = new NativeArray<float3>(4, Allocator.TempJob);
        ...
        if(s == 19 || s == 20){
            ...
            {
              fv[i] = (floatverts[19][faceTriangles[dir][i]] * scale) + pos;
            }
            return fv;
        }
        else{
            for (int i = 0; i < fv.Length; i++)
            {
              fv[i] = (floatverts[(int)s][faceTriangles[dir][i]] * scale) + pos;
            }
            return fv;
        }
    }
